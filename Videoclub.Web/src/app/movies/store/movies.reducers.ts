...
export const moviesReducers = createReducer(
    ...
    on(MoviesActions.FilterMovies, (state) => {
        ...
        if (state.categoryToFilter !== "All") {
            filteredMovies = filteredMovies.filter((movie) => movie.category === state.categoryToFilter);
        }
        if (state.stateToFilter !== "All") {
            filteredMovies = filteredMovies.filter((movie) => movie.state === state.stateToFilter);
        }
        if (state.textToFind !== "") {
            filteredMovies = filteredMovies.filter((movie) => movie.title.toLowerCase().includes(state.textToFind.toLowerCase()));
        }
        return {
            ...state,
