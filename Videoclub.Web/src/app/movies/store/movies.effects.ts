    ...
    this.actions$.pipe(
        ...
        mergeMap(() => {
            return this.moviesService.getMovies().pipe(
                map((movies) => 
                    MoviesActions.LoadAllMoviesSuccess({movies})
                ),
                catchError((error) => of(MoviesActions.LoadAllMoviesFail({ error: error.error }))))
        }),
        ));
    ...
    this.actions$.pipe(
        ...
        mergeMap(() => {
            return this.moviesService.getCategories().pipe(
                ...
                catchError((error) => of(MoviesActions.LoadCategoriesFail({ error: error.error }))))
        })));

    filterMovies$ = createEffect(() =>
    this.actions$.pipe(
        ofType(MoviesActions.UpdateTextToFind, MoviesActions.UpdateCategoryToFilter, MoviesActions.UpdateStateToFilter),
        ...
        ofType(MoviesActions.RentMovie),
         mergeMap((action) => {
            return this.moviesService.rentMovie(action.history).pipe(
                map((movies) => 
                    MoviesActions.RentMovieSuccess({movies})
                ),
                catchError((error) => of(MoviesActions.RentMovieFail({ error: error.error }))))
        }))); 
        ...
        ofType(MoviesActions.ReturnMovie),
         mergeMap((action) => {
            return this.moviesService.returnMovie(action.movie_id).pipe(
                map((movies) => 
                    MoviesActions.ReturnMovieSuccess({movies})
                ),
                catchError((error) => of(MoviesActions.ReturnMovieFail({ error: error.error }))))
        })));
        ...
        ofType(MoviesActions.UpdateMovie),
         mergeMap((action) => {
            return this.moviesService.updateMovie(action.movie).pipe(
                map((movies) => 
                    MoviesActions.UpdateMovieSuccess({movies})
                ),
                catchError((error) => of(MoviesActions.UpdateMovieFail({ error: error.error }))))
        })));
        ...
        ofType(MoviesActions.CreateMovie),
         mergeMap((action) => {
            return this.moviesService.createMovie(action.movie).pipe(
                map((movies) => 
                    MoviesActions.CreateMovieSuccess({movies})
                ),
                catchError((error) => of(MoviesActions.CreateMovieFail({ error: error.error }))))
        })));
        ...
        ofType(MoviesActions.DeleteMovie),
         mergeMap((action) => {
            return this.moviesService.deleteMovie(action.movie_id).pipe(
                map((movies) => 
                    MoviesActions.DeleteMovieSuccess({movies})
                ),
                catchError((error) => of(MoviesActions.DeleteMovieFail({ error: error.error }))))
        })));
