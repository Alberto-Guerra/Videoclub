    ...
    this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.service.login(action.user, action.password).pipe(
          ...
          catchError((error) =>
            of(AuthActions.loginFail({ error: error.error }))
          )
        );
      })
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.service.register(action.user).pipe(
          ...
          catchError((error) =>
            of(AuthActions.registerFail({ error: error.error }))
          )
        );
      })
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.service.autoLogin(action.token).pipe(
          ...
          catchError((error) =>
            of(AuthActions.autoLoginFail({ error: error.error }))
          )
        );
      })
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap(() => {
        return this.service.logout().pipe(
          ...
          catchError((error) =>
            of(AuthActions.logoutFail({ error: error.error }))
          )
        );
      })
    )
